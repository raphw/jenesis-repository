 ...
 * container. By default an unmet requirement skips the test (a JUnit assumption), so a developer machine without
 * Docker stays green. When the test JVM carries {@code -Djenesis.test.required} (injected by the {@code ci}
 * profile's process-override properties), an unmet requirement fails the test instead: CI installs every tool the
 * selected suites need, so a skip there is a broken lane hiding as green, never an acceptable outcome.
 */
