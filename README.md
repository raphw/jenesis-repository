...
| `build.jenesis.repository.format.maven`    | `format/maven`        | The Maven / Jenesis-module format (`/maven/...`, `/module/...`, `/artifact/...`): the dual layout, cross-publishing, POM generation, and `maven-metadata.xml` generated on read. Builds on the core's `Publication` / `MavenMetadata`. |
