...
| `build.jenesis.repository.format.java`     | `format/java`         | The shared Java repository-layout primitives the Maven and Jenesis layouts build on: reading a jar's module name and mapping it to a Maven coordinate (`JavaLayout`), and POM generation (`PomGenerator`). It also carries the cross-publish bridge (`ModuleView` / `MavenView`) - exported *only* to those two modules, so cross-publishing stays off the public SPI. |
| `build.jenesis.repository.format.maven`    | `format/maven`        | The Maven layout (`/maven/...`): stores the blob, generates `maven-metadata.xml` on read, proxies Maven Central. A modular jar is cross-published into the Jenesis layout over the bridge; it `provides` the `MavenView` (a module's Maven view, POM and all) that the Jenesis layout uses. |
| `build.jenesis.repository.format.jenesis`  | `format/jenesis`      | The Jenesis module layout (`/module/...`, `/artifact/...`): stores and serves modules over the same content-addressed blob. A published module is cross-published into the Maven layout over the bridge; it `provides` the `ModuleView` (a modular jar's module view) that the Maven layout uses. |
...
| `build.jenesis.repository`          | `server/repository`   | The dispatcher, format-neutral: it `uses RepositoryFormat`, loads every format via `ServiceLoader`, scopes the store, and enforces auth - with no knowledge of any layout, so it serves a fully capable repository even with no format on the module path (every request 404s until one is). The format-neutral content-addressed store (`Publication`) and the optional pull-through proxy live here; the Maven and Jenesis layouts and their cross-publishing are plugin modules. Basic age/size retention. |
